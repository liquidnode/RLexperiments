...
class VirtualPG():
    def __init__(self, state_shape, action_dict, args, time_deltas, only_visnet=False):
        ...
        self.cost_of_information_actions = 1e-4
    ...
    def _make_model(self):
        ...
        modules['actions_dec'] = FullDiscretizerWLast(self.actions_embed_dim + self.cnn_embed_dim + self.memory_embed + self.state_dim, self.device, self.args, self.action_dict)
        ...
        modules['state_predict'] = torch.nn.Sequential(torch.nn.Linear(400, 400),
                                                       Lambda(lambda x: swish(x)))
        modules['Q_value'] = torch.nn.Sequential(torch.nn.Linear(self.actions_embed_dim + self.cnn_embed_dim + self.memory_embed + self.state_dim, self.args.hidden),
                                                 Lambda(lambda x: swish(x)),
    ...
    def pretrain(self, input):
        ...
        rdict['inter_rec_loss'] = inter_rec_loss
        ...
        #Q value stuff
        #current Q
        Q = self._model['modules']['Q_value'](torch.cat([current_pov_embed,
                                                         current_actions_embed,
                                                         current_memory_embed,
                                                         cat_current_state], dim=-1))[:,0]
        ...
        next_Q = self._model['modules']['Q_value'](torch.cat([next_pov_embed.detach(),
                                                              next_actions_embed.detach(),
                                                              next_memory_embed.detach(),
                                                              cat_next_state], dim=-1))[:,0]
    ...
    def prediction_loss(self, current_pov_embed, current_memory_embed, current_state, current_actions, reward):
        ...
        if True:
            ...
            comp_bc_loss, _, _, _, _ = self._model['modules']['actions_dec'].get_loss_BC(torch.cat([tembed, actions_embed], dim=-1), current_action)
            loss += comp_bc_loss
        ...
        bc_loss, _, _, _, _ = self._model['modules']['actions_dec'].get_loss_BC(torch.cat([tembed, pred_actions_embed], dim=-1), current_action)
        #for p in self._model['modules']['actions_dec'].parameters():
    ...
    def select_action(self, state, batch=False):
        ...
        with torch.no_grad():
            ...
            r_action = {}
            o_action = {}
            r_action, conaction, entropy = self._model['modules']['actions_dec'].sample(torch.cat([tembed, pred_actions_embed], dim=-1))
            for a in r_action:
