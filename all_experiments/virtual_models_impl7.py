...
class VirtualPG():
                    ...
                    if s.shape != state_dict[n].shape:
                        if False:
                            ...
                            if len(s.shape) == 2:
                                A = min(s.shape[0],state_dict[n].shape[0])
                                B = min(s.shape[1],state_dict[n].shape[1])
                                n_sa[:A, :B] = state_dict[n][:A, :B]
                            else:
                                n_sa[:min(s.shape[0],state_dict[n].shape[0])] = state_dict[n][:min(s.shape[0],state_dict[n].shape[0])]
                            state_dict[n] = n_sa
                self.load_state_dict(state_dict, False)
            else:
                state_dict = {n: v for n, v in state_dict.items() if not n.startswith('state_predict.')}
                self.load_state_dict(state_dict, False)
    ...
    def _make_model(self):
        ...
        #infoDIM loss
        if INFODIM:
            modules['disc'] = torch.nn.Sequential(torch.nn.Linear(self.cnn_embed_dim*2+self.actions_embed_dim, self.args.hidden), 
        ...
        #state prediction
        self.random_dim = 64
        modules['state_predict'] = torch.nn.Sequential(torch.nn.Linear(self.args.hidden+self.random_dim, self.args.hidden), #input is all_embed
                                                       Lambda(lambda x: swish(x)),
                                                       torch.nn.Linear(self.args.hidden, self.args.hidden), #input is all_embed
                                                       Lambda(lambda x: swish(x)),
                                                       torch.nn.Linear(self.args.hidden, self.cnn_embed_dim))
    ...
    def pretrain(self, input):
        ...
        state_pred_loss, mean, std, dis_loss, gen_loss = self.state_prediction(all_embed, next_embed_no_tanh, actions_embed, current_embed_no_tanh)
        ...
        if RECONSTRUCTION:
            ...
            if False:
                state_pred_sample = torch.distributions.Normal(mean, std).sample()
                rec_pov2 = self._model['modules']['generate_pov'](tanh_or_not(state_pred_sample))
                rec_loss2 = self._model['loss'](rec_pov2, states_gen_embed[:,1]) * 2.0
                loss += rec_loss2
    ...
    def state_prediction(self, all_embed, next_embed_no_tanh, actions_embed, current_embed_no_tanh):
        OLD_STUFF = False
        if OLD_STUFF:
            ...
            log_prob = dist.log_prob(next_embed_no_tanh.detach())
        else:
            ran_sample = torch.zeros([all_embed.shape[0], self.random_dim], device=all_embed.device).normal_()
            mean = self._model['modules']['state_predict'](torch.cat([all_embed, ran_sample], dim=-1))
        
        if INFODIM:
            real_pairs = torch.cat([actions_embed, tanh_or_not(current_embed_no_tanh), tanh_or_not(next_embed_no_tanh).detach()], dim=-1)
            if False:
                fake_pairs1 = torch.cat([actions_embed, tanh_or_not(current_embed_no_tanh).detach(), tanh_or_not(dist.sample() if OLD_STUFF else mean).detach()], dim=-1)
                random_roll = np.random.randint(1, self.args.er-1)
                fake_pairs2 = torch.cat([actions_embed, tanh_or_not(current_embed_no_tanh).roll(random_roll,0), tanh_or_not(next_embed_no_tanh)], dim=-1)
                random_roll = np.random.randint(1, self.args.er-1)
                fake_pairs3 = torch.cat([actions_embed.roll(random_roll,0), tanh_or_not(current_embed_no_tanh), tanh_or_not(next_embed_no_tanh)], dim=-1)
                fake_pairs = torch.cat([fake_pairs1, fake_pairs2, fake_pairs3], dim=0)
            else:
                fake_pairs = torch.cat([actions_embed.detach(), tanh_or_not(current_embed_no_tanh).detach(), tanh_or_not(dist.sample() if OLD_STUFF else mean).detach()], dim=-1)
            ...
            if self.train_iter % 8 == 0:
                fake_pairs = torch.cat([actions_embed, tanh_or_not(current_embed_no_tanh), tanh_or_not(dist.sample() if OLD_STUFF else mean)], dim=-1)
        ...
        if OLD_STUFF:
            return -log_prob.mean() + dis_loss + gen_loss, mean, std, dis_loss, gen_loss
        else:
            return dis_loss + gen_loss + self._model['loss'](mean, next_embed_no_tanh.detach()), mean, mean, dis_loss, gen_loss
    ...
    def prediction_loss(self, pov_embed, memory_embed, current_state, current_actions, reward):
        ...
        std = std.exp()+1e-12
        dist = torch.distributions.Normal(mean, std)
        log_prob = dist.log_prob(actions_embed_no_tanh.detach())#(0.1 * actions_embed_no_tanh + 0.9 * actions_embed_no_tanh.detach())
        ...
        return loss, all_embed, actions_embed, {'action_comp_loss': action_compression_loss,
    ...
    def select_action(self, state, batch=False):
        ...
        with torch.no_grad():
            ...
            if True:
                ...
                #just use mean
                sample = mean
            actions_embed = tanh_or_not(sample)
            #decode actions
            actions_logits = self._model['modules']['actions_dec'](torch.cat([tembed, actions_embed], dim=-1))
            actions_logits = actions_logits.view(-1, self.max_predict_range, self.action_dim if not AUX_CAMERA_LOSS else self.action_dim + 5)
            if AUX_CAMERA_LOSS:
