#
#this file is modified code from https://github.com/facebookresearch/XLM/blob/master/src/model/memory/memory.py
#
...
class HashingMemory(nn.Module):
    ...
    def precalc_forward(self):
        output = self.values(
            self.indices,
            per_sample_weights=self.scores.to(self.values.weight.data)
        ).to(self.scores)                                                         # (bs, v_dim)
        output = F.dropout(output, p=self.value_dropout, training=self.training)  # (bs, v_dim)

        # reshape output
        if self.input2d:
            output = output.view(self.prefix_shape + (self.v_dim,))             # (n_images, width, height, v_dim)
            output = output.transpose(1, 3)                                       # (n_images, v_dim, height, width)
        else:
            if len(self.prefix_shape) >= 2:
                output = output.view(self.prefix_shape + (self.v_dim,))                # (..., v_dim)
    ...
    def register_args(parser):
        ...
        parser.add_argument("--mem_k_dim", type=int, default=128,
        ...
        parser.add_argument("--mem_heads", type=int, default=2,
                            help="Number of memory reading heads")
        parser.add_argument("--mem_knn", type=int, default=64,
        ...
        parser.add_argument("--mem_n_keys", type=int, default=512, #orig 256
                            help="Number of keys")
