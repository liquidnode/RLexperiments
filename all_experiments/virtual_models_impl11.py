...
class GenNet(torch.nn.Module):
    def __init__(self, args, state_shape, embed_dim):
        ...
        t_layers.append(torch.nn.Sigmoid())
        layers.append(torch.nn.Sequential(*t_layers))
...
class VirtualPG():
    def __init__(self, state_shape, action_dict, args, time_deltas):
        ...
        self.free_nats = 3.0
        self.cost_of_information_pov = 2e-5
        self.cost_of_information_actions = 2e-5
    ...
    def loadstore(self, filename, load=True):
        if load:
            ...
            #state_dict = {n: v for n, v in state_dict.items() if not n.startswith('Q_value')}
    ...
    def _make_model(self):
        ...
        modules['generate_pov'] = GenNet(self.args, self.state_shape, 400)

        
        #state prediction
        modules['state_predict_hidden'] = torch.nn.Sequential(torch.nn.Linear(self.actions_embed_dim + self.cnn_embed_dim + self.memory_embed + self.state_dim + 1, 400),
                                                       SkipHashingMemory(400, self.args),
                                                       Lambda(lambda x: swish(x)))
                                                       #torch.nn.Linear(self.args.hidden, self.args.hidden),
                                                       #Lambda(lambda x: swish(x)))
        modules['state_predict'] = torch.nn.Sequential(torch.nn.Linear(400, 400),
                                                       Lambda(lambda x: swish(x)))
        modules['Q_value'] = torch.nn.Sequential(torch.nn.Linear(400, self.args.hidden),
                                                 Lambda(lambda x: swish(x)),
    ...
    def pretrain(self, input):
        ...
        if False:
            state_predict_hidden = self._model['modules']['state_predict_hidden'](torch.cat([current_pov_embed,
                                                                                             ...
                                                                                             cat_current_state,
                                                                                             torch.zeros([cat_current_state.shape[0], 1], device=cat_current_state.device, dtype=torch.float32)], dim=-1))
            state_predict_hidden = self._model['modules']['state_predict'](state_predict_hidden)
            rec_pov = self._model['modules']['generate_pov'](state_predict_hidden)
        ...
        state_predict_hidden = self._model['modules']['state_predict_hidden'](torch.cat([current_pov_embed,
                                                                                         ...
                                                                                         r_offset], dim=-1))
        state_predict_hidden = self._model['modules']['state_predict'](state_predict_hidden)
        inter_rec_pov = self._model['modules']['generate_pov'](state_predict_hidden)
        ...
        inter_rec_loss = F.binary_cross_entropy(inter_rec_pov, states_gen_embed[:,2] * 0.5 + 0.5)
        loss += inter_rec_loss * 2.0
        with torch.no_grad():
            rdict['inter_rec_loss'] = inter_rec_loss - F.binary_cross_entropy(states_gen_embed[:,2] * 0.5 + 0.5, states_gen_embed[:,2] * 0.5 + 0.5)
        ...
        Q = self._model['modules']['Q_value'](state_predict_hidden_last)[:,0]
        ...
        with torch.no_grad():
            ...
            next_Q = self._model['modules']['Q_value'](state_predict_hidden_last)[:,0]
        ...
        Q_loss = (Q - (0.8 * target.detach() + 0.2 * Q.detach())).pow(2).mean() * 10.0
        with torch.no_grad():
        ...
        if True:
            ...
            preP = np.concatenate([(states_gen_embed[0,0].permute(1, 2, 0)*0.5+0.5).data.cpu().numpy(),
                                    rec_pov[0].permute(1, 2, 0).data.cpu().numpy()], axis=1)
            aftP = np.concatenate([(states_gen_embed[0,2].permute(1, 2, 0)*0.5+0.5).data.cpu().numpy(),
                                    inter_rec_pov[0].permute(1, 2, 0).data.cpu().numpy()], axis=1)
    ...
    def prediction_loss(self, current_pov_embed, current_memory_embed, current_state, current_actions, reward):
        ...
        if False:
            pred_actions_kl = self.normal2_kl_divergence(pred_actions_embed_mean, pred_actions_embed_log_var, actions_embed_mean.detach(), actions_embed_log_var.detach())
            loss += 1e-7 * pred_actions_kl
