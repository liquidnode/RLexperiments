...
class VisualNetVariational(torch.nn.Module):
    ...
    def forward(self, state_pov):
        ...
        if len(state_pov.shape) == 5:
            ...
            x = state_pov.view(-1, self.state_shape['pov'][2], self.state_shape['pov'][0], self.state_shape['pov'][1])
        else:
            x = state_pov
...
class VirtualPGVAE():
    def __init__(self, state_shape, action_dict, args, time_deltas):
        ...
        self.cont_capacity = (0.0, 15.0, 1e5, 50.0)
        self.disc_capacity = (0.0, 10.0, 1e5, 50.0)
    ...
    def loadstore(self, filename, load=True):
        if load:
            ...
            #torch.load(self.train_iter, filename + 'train_iter')
        else:
    ...
    def pretrain(self, input):
        ...
        #sample embeds
        with torch.no_grad():
            embeds = self._model['modules']['pov_embed'].sample(states['pov_embed_mu'], states['pov_embed_log_var'], states['pov_embed_log_alphas'], False)

        #randomly choose frames from the past to generate
        states_gen_embed = states['pov'][:,self.zero_time_point].unsqueeze(1)
        self.current_slice = 0
        if self.num_random_slices > 0:
            with torch.no_grad():
                rslices = np.random.choice(self.zero_time_point, shape=(self.args.er, self.num_random_slices))
                #is there a better solution?
                rslices = variable(rslices, True).long()[:,:,None,None,None].expand(-1, -1, self.state_shape['pov'][2], self.state_shape['pov'][0], self.state_shape['pov'][1])
                states_gen_embed = torch.cat([states['pov'].gather_(1, rslices), states_gen_embed], dim=1)
                self.current_slice = self.num_random_slices
        #generate embedding
        mu, log_var, discs = self._model['modules']['pov_embed'](states_gen_embed)
        gen_embed = self._model['modules']['pov_embed'].sample(mu, log_var, discs)
        #redistribute embeddings
        if self.num_random_slices > 0:
            rslices = torch.cat([rslices, torch.tensor([[self.zero_time_point]], device=self.device, dtype=torch.int64)[:,:,None].expand(self.args.er,-1,self.cnn_embed_dim)], dim=1)
        else:
            rslices = torch.tensor([[self.zero_time_point]], device=self.device, dtype=torch.int64)[:,:,None].expand(self.args.er,-1,self.cnn_embed_dim)
        embeds.scatter_(1, rslices, gen_embed)

        #get current memory_embed
        memory_embed, _, memory_hidden = self.get_memory_embed(embeds[:,:(self.zero_time_point+1)])
        #get current final state embed
        current_hidden = self.get_state_hidden(current_state, embeds[:,self.zero_time_point].unsqueeze(1), memory_embed.unsqueeze(1))
        #make current prediction loss
        loss, rdict = self.prediction_loss(embeds[:,self.zero_time_point].unsqueeze(1), current_hidden, current_action, reward[:,self.zero_time_point:], current_lastaction, is_weight)
        ...
        #make infoDIM loss
        dis_loss = self.get_infoDIM_loss(embeds[:,self.zero_time_point], embeds[:,(self.zero_time_point+1)])
        loss += dis_loss
    ...
    def reset(self):
        self.hidden = torch.zeros((1,self.memory_embed), device=self.device, dtype=torch.float32, requires_grad=False)
    ...
    def select_action(self, state, batch=False):
        ...
        with torch.no_grad():
            ...
            #memory_embed
            memory_embed, self.hidden = self._model['modules']['memory'](gen_embed, self.hidden)
            #all embed
            tembed = [pov_embed, memory_embed]
            for o in self.state_shape:
                if o in self.state_keys and o != 'pov':
