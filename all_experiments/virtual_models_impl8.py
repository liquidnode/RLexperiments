...
class VirtualPG():
    ...
    def _make_model(self):
        ...
        #infoDIM loss
        if INFODIM:
            modules['disc'] = torch.nn.Sequential((torch.nn.Linear(self.cnn_embed_dim*2+self.actions_embed_dim+1, self.args.hidden)), 
                                                  Lambda(lambda x: swish(x)),#torch.nn.LeakyReLU(0.1), 
                                                  (torch.nn.Linear(self.args.hidden, self.args.hidden)), 
                                                  Lambda(lambda x: swish(x)),
                                                  (torch.nn.Linear(self.args.hidden, 1)))#
            modules['disc_actions'] = torch.nn.Sequential((torch.nn.Linear(self.cnn_embed_dim + self.memory_embed + self.state_dim + self.actions_embed_dim, self.args.hidden)),
                                                  Lambda(lambda x: swish(x)),#torch.nn.LeakyReLU(0.1), 
                                                  (torch.nn.Linear(self.args.hidden, 1)))#
        ...
        #state prediction
        modules['state_predict'] = torch.nn.Sequential(torch.nn.Linear(self.args.hidden+self.random_dim+1, self.args.hidden), #input is all_embed
                                                       Lambda(lambda x: swish(x)),
    ...
    def pretrain(self, input):
        ...
        state_pred_loss, mean, std, dis_loss, gen_loss = self.state_prediction(all_embed, next_embed_no_tanh, actions_embed, current_embed_no_tanh, r_offset)
        ...
        if RECONSTRUCTION:
            ...
            if True:
                ...
                rec_loss2 = self._model['loss'](rec_pov2, states_gen_embed[:,1]) * 5.0
                loss += rec_loss2
    ...
    def state_prediction(self, all_embed, next_embed_no_tanh, actions_embed, current_embed_no_tanh, r_offset):
        ...
        if OLD_STUFF:
            ...
            mean = self._model['modules']['state_predict'](torch.cat([all_embed, ran_sample, r_offset], dim=-1))
        
        if INFODIM:
            real_pairs = torch.cat([actions_embed, tanh_or_not(current_embed_no_tanh), tanh_or_not(next_embed_no_tanh).detach(), r_offset], dim=-1)
            if False:
                fake_pairs1 = torch.cat([actions_embed, tanh_or_not(current_embed_no_tanh).detach(), tanh_or_not(dist.sample() if OLD_STUFF else mean).detach(), r_offset], dim=-1)
                random_roll = np.random.randint(1, self.args.er-1)
                fake_pairs2 = torch.cat([actions_embed, tanh_or_not(current_embed_no_tanh).roll(random_roll,0), tanh_or_not(next_embed_no_tanh), r_offset], dim=-1)
                random_roll = np.random.randint(1, self.args.er-1)
                fake_pairs3 = torch.cat([actions_embed.roll(random_roll,0), tanh_or_not(current_embed_no_tanh), tanh_or_not(next_embed_no_tanh), r_offset], dim=-1)
                fake_pairs = torch.cat([fake_pairs1, fake_pairs2, fake_pairs3], dim=0)
            else:
                fake_pairs = torch.cat([actions_embed.detach(), tanh_or_not(current_embed_no_tanh).detach(), tanh_or_not(dist.sample() if OLD_STUFF else mean).detach(), r_offset], dim=-1)
            ...
            if self.train_iter % 5 == 0:
                fake_pairs = torch.cat([actions_embed.detach(), tanh_or_not(current_embed_no_tanh).detach(), tanh_or_not(dist.sample() if OLD_STUFF else mean), r_offset], dim=-1)
        ...
        if OLD_STUFF:
            ...
            return (dis_loss + gen_loss) * 0.1, mean, mean, dis_loss, gen_loss
