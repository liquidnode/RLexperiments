...
class VirtualReplayBufferPER():
    def __init__(self, args, data_description, input_queue, prio_queues, time_deltas, max_range=0, blocking=True):
        num_workers = args.num_replay_workers
        if not blocking:
            num_workers = 1
            #TODO demultiplexer einbauen
        ...
        params = [(args, i, data_description, input_queue[i], self.batch_queues[i], prio_queues[i], time_deltas, max_range, blocking) 
                  for i in range(num_workers)]
    ...
    @staticmethod
    def worker(args, pid, data_description, input_queue, batch_queue, prio_queue, time_deltas, max_range, blocking):
        try:
            num_workers = args.num_replay_workers
            sample_efficiency = args.sample_efficiency
            chunk_size = args.chunk_size
            ...
            alpha = 1.0
            beta = 0.9
            beta_increment_per_sampling = 0.0
            global max_error
            max_error = eps ** alpha
            global sum_tree
                    ...
                    while j < sample_efficiency:
                        ...
                        for i in range(batch_size):
                            ...
                            while notFound and num_attempts < 10:
                                ...
                                if not ((idx - write_pos) % buffer_size < (idx - write_pos + max_skip) % buffer_size):
                                    num_attempts += 1
                                    continue
                                priorities.append(p)
                        ...
                        for k in data_description:
                            if not data_description[k]['compression']:
                                if k == 'reward' or k == 'done':
                                    trange = range(0, max(max(time_deltas),max_range))
                                    ridxs = np.stack([idxs+s for s in trange], axis=1).reshape([-1])%buffer_size
                                    rbatch = buffers[k][ridxs]
                                    rbatch = rbatch.reshape([batch_size, max(trange)+1]+list(rbatch.shape[1:]))
                                    if k == 'done' and np.any(rbatch[:,:-1]) and avoid_done:
                                        continue
                                else:
                                    ridxs = np.stack([idxs+s for s in time_deltas], axis=1).reshape([-1])%buffer_size
                                    rbatch = buffers[k][ridxs]
                                    rbatch = rbatch.reshape([batch_size, len(time_deltas)]+list(rbatch.shape[1:]))
                                batch[k] = SharedMemory(rbatch)
                                batch[k] = batch[k].shared_memory()
                            else:
                                ridxs = np.stack([idxs+s for s in time_deltas], axis=1).reshape([-1])%buffer_size
