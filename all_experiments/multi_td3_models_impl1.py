...
class MultiTD3():
    def __init__(self, state_shape, action_dict, args, time_deltas, only_visnet=False):
        ...
        self.free_nats = 3.0
        self.cost_of_information_pov = 2e-4
        self.cost_of_information_actions = 8e-4
        ...
        if load:
            ...
            if self.args.pretrained_load is not None:
                ...
                if self.only_visnet:
                    ...
                    state_dict = {n: v for n, v in state_dict.items() if not n.startswith('state') and not n.startswith('generate_pov')}
                    nstate_dict = {}
                    for n, v in state_dict.items():
                        if n.startswith('Q_value'):
                            nstate_dict[n.replace('Q_value', 'Q_valueA')] = v
                            nstate_dict[n.replace('Q_value', 'Q_valueB')] = v
                        else:
                            nstate_dict[n] = v
    ...
    def train(self, input, worker_num=None):
        ...
        comp_bc_loss, _, _, _, _ = self._model['modules']['actions_dec'].get_loss_BC(torch.cat([tembed, current_actions_embed], dim=-1), current_action)
        loss += comp_bc_loss
        ...
        if self.train_iter % POLICY_FREQ == 0:
            #actor loss
            #predict action
            pred_actions_embed_mean, pred_actions_embed_log_var = torch.split(self._target['actions_embed_predict'](tembed), self.actions_embed_dim, dim=-1)
            pred_actions_embed = sample_gaussian(pred_actions_embed_mean, pred_actions_embed_log_var)

            #action compression loss
            actions_kl = self.normal1_kl_divergence(pred_actions_embed_mean, pred_actions_embed_log_var)
            loss += actions_kl * self.cost_of_information_actions
