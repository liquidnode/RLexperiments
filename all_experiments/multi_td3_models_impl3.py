...
class MultiTD3():
    def __init__(self, state_shape, action_dict, args, time_deltas, only_visnet=False):
        ...
        self.cost_of_information_pov = 100.0
        self.cost_of_information_actions = 1000.0
    ...
    def loadstore(self, filename, load=True):
        if load:
            ...
            if self.args.pretrained_load is not None:
                ...
                if self.only_visnet:
                    state_dict = {n: v for n, v in state_dict.items() if n.startswith('pov_embed.')}
                if np.any([n.startswith('v_predict') for n in state_dict]) and not self.args.pretrain:
                    state_dict = {n: v for n, v in state_dict.items() if not n.startswith('v_predict')}
                    nstate_dict = {}
                    for n, v in state_dict.items():
                        if n.startswith('Q_valueA'):
    ...
    def _train(self, input, pretrain, worker_num=None):
        ...
        pov_mmd = compute_mmd_gauss(current_pov_embed)
        rdict = {}
        rdict['pov_mmd'] = pov_mmd
        pov_kl_loss = self.cost_of_information_pov * pov_mmd
        ...
        current_actions_embed = self._model['modules']['actions_enc'](tembed_wactions)
        actions_mmd = compute_mmd_gauss(current_actions_embed)
        loss += self.cost_of_information_actions * actions_mmd
        rdict['actions_mmd'] = actions_mmd
        with torch.no_grad():
            rdict['actions_mean'] = current_actions_embed.mean()
            rdict['actions_std'] = current_actions_embed.std(dim=0).mean()
        ...
        if pretrain:
            ...
            if True:
                ...
                loss += self.cost_of_information_pov * compute_mmd_gauss(next_pov_embed)
                ...
                pred_actions_embed = self._model['modules']['actions_enc'](nembed_wactions)
                loss += self.cost_of_information_actions * compute_mmd_gauss(pred_actions_embed)
            ...
            if is_weight is None:
                Q_loss = (Q - target).pow(2).mean() * 4.0
            else:
                Q_loss = ((Q - target).pow(2) * is_weight).mean() * 4.0
            with torch.no_grad():
            ...
            actions_pred_loss = -self.normal_log_prob_dens(pred_actions_embed_mean2, pred_actions_embed_log_var2, current_actions_embed.detach()).mean()
            loss += actions_pred_loss * 0.1
            rdict['pred_actions'] = actions_pred_loss
