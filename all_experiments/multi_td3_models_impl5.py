...
class MultiTD4():
    ...
    def _make_model(self):
        ...
        if Q_PARAM:
            ...
            modules['actions_embed_predict'] = torch.nn.Sequential(torch.nn.Linear(self.cnn_embed_dim + self.memory_embed + self.state_dim, self.args.hidden),
                                                                   Lambda(lambda x: swish(x)),
                                                                   torch.nn.Linear(self.args.hidden, self.actions_embed_dim))#mean and Q value parameter
        ...
        if INFODIM:
            modules['actions_embed_disc'] = torch.nn.Sequential(torch.nn.Linear(self.actions_embed_dim, self.args.hidden),
                                                                Lambda(lambda x: swish(x)),
                                                                torch.nn.Linear(self.args.hidden, 1))
    ...
    def train(self, input, worker_num=None):
        ...
        with torch.no_grad():
            next_pov_embed = self._target['pov_embed'](states['pov'][:,1])
        ...
        if STAGE1:
            ...
            #direct decode loss
            #det_current = current_actions_embed.detach()
            single_dec_loss, _, _, _, _ = self._model['modules']['actions_dec_single'].get_loss_BC(current_actions_embed.detach(), current_action, last_action)
        ...
        with torch.no_grad():
            next_V = self._target['value'](nembed.detach())[:,0]
        if STAGE1:
        ...
        if is_weight is None:
            Q_loss = (current_Q - target.detach()).pow(2).mean()
        else:
            Q_loss = ((current_Q - target.detach()).pow(2) * is_weight).mean()
        loss += Q_loss
        with torch.no_grad():
            rdict['Q_diff'] = torch.abs(current_Q - target).mean()
            rdict['current_Q_mean'] = current_Q.mean()
            rdict['max_Q_mean'] = current_V.mean()
            rdict['target'] = target.mean()
            
        #consistency loss
        decoded_best_action = self._model['modules']['actions_dec_all'](current_actions_embed)
        decoded_best_action = decoded_best_action.view(-1, self.max_predict_range, self.action_dim)
        decoded_best_action = torch.split(decoded_best_action, self.action_split, -1)
        actions_camera = torch.tanh(decoded_best_action[0])
        decoded_best_action = list(decoded_best_action[1:])
        for l in range(len(decoded_best_action)):
            #decoded_best_action[l] = torch.nn.Softmax(-1)(decoded_best_action[l])
            decoded_best_action[l] = F.gumbel_softmax(decoded_best_action[l], self.args.temperature, True)
        r_decoded_best_action = []
        i = 0
        for a in self.action_dict:
            if a != 'camera':
                r_decoded_best_action.append(decoded_best_action[i].view(-1, self.action_dict[a].n*self.max_predict_range))
                i += 1
            else:
                r_decoded_best_action.append(actions_camera.view(-1, 2*self.max_predict_range))
        r_decoded_best_action = torch.cat(r_decoded_best_action, dim=-1)
        r_encoded_best_action = self._model['modules']['actions_enc'](r_decoded_best_action)
        consistency_loss = (r_encoded_best_action - current_actions_embed).pow(2).sum(1)
        if is_weight is None:
            consistency_loss = consistency_loss.mean()
        else:
            consistency_loss = (consistency_loss * is_weight).mean()
        loss += consistency_loss
        rdict['consistency_loss'] = consistency_loss


        if INFODIM:
            #discr. action_embeds
            real_scores = self._model['modules']['actions_embed_disc'](current_actions_embed.detach())[:,0]
            fake_scores = self._model['modules']['actions_embed_disc'](current_best_action.detach())[:,0]
            ...
            for p in self._model['modules']['actions_embed_disc'].parameters():
                p.requires_grad_(False)
            fake_scores = self._model['modules']['actions_embed_disc'](current_best_action)[:,0]
            for p in self._model['modules']['actions_embed_disc'].parameters():
        ...
        if self.running_Q_diff is None:
            self.running_Q_diff = rdict['Q_diff']
        self.running_Q_diff = 0.1 * rdict['Q_diff'] + 0.9 * self.running_Q_diff
        if self.train_iter % 200 == 0 and self.running_Q_diff < 0.05:
            # Update the frozen target models
            for param, target_param in zip(self._model['modules'].parameters(), self._target.parameters()):
                target_param.data.copy_(param.data)#TAU * param.data + (1 - TAU) * target_param.data)
...
def main():
    ...
    parser.add_argument("--agent_from", type=str, default='multi_td3_models_impl5', help="Model python module")
    ...
    args.load = None#"treechop/model_stage2_5"
    args.save = "treechop/model_stage2_5"
