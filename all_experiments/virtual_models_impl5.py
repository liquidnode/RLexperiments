...
class GenNet(torch.nn.Module):
    ...
    def forward(self, state):
        ...
        if len(state.shape) == 3:
            ...
            x = state.contiguous().view(-1, state.shape[1]*state.shape[2])
        else:
            x = state
...
class VirtualPGVAE():
    def __init__(self, state_shape, action_dict, args, time_deltas):
        ...
        self.disc_embed_dims = [2 for i in range(16)]
        ...
        #self.capacity = (0.0, 50.0, 2e5, 3.0)
        self.capacity = (100.0, 30.0, 2e5, 0.0)
        self.auto_capacity = False
        self.capacity_delta_target = 0.1
    ...
    def loadstore(self, filename, load=True):
        if load:
            ...
            if self.auto_capacity:
                self.current_capacity = torch.load(filename + 'current_capacity')
            ...
            if self.auto_capacity:
                torch.save(self.current_capacity, filename + 'current_capacity')
    ...
    def _make_model(self):
        ...
        modules['state_predict'] = torch.nn.Linear(self.memory_embed, self.continuous_embed_dim*2+self.total_disc_embed_dim)
    ...
    def pretrain(self, input):
        ...
        #sample embeds
        with torch.no_grad():
            past_embeds = self._model['modules']['pov_embed'].sample(states['pov_embed_mu'][:,:self.zero_time_point], states['pov_embed_log_var'][:,:self.zero_time_point], states['pov_embed_log_alphas'][:,:self.zero_time_point], False)
        ...
        memory_embed = self.get_memory_embed(past_embeds, current_embed[:,0])
        ...
        if False:
            if not self.auto_capacity:
                ...
                kl = kl_disc + kl_cont
                kl_min, kl_max, kl_num_iters, kl_gamma = \
                        self.capacity
                if self.auto_capacity:
                    kl_cap_current = self.current_capacity
                else:
                    kl_cap_current = (kl_max - kl_min) * self.train_iter / float(kl_num_iters) + kl_min
                    kl_cap_current = min(kl_cap_current, max(kl_min, kl_max))
                    kl_cap_current = max(kl_cap_current, min(kl_min, kl_max))
                capacity_loss = kl_gamma * torch.nn.ReLU()(kl - kl_cap_current).mean()
                loss += capacity_loss
            else:
                kl_cont = self.normal1_kl_divergence(mu, log_var)
                kl_disc = self.discrete1_kl_divergence(discs)
                ...
                kl_min, kl_max, kl_num_iters, kl_gamma = \
                        self.capacity
                kl_cap_current = kl.mean().detach()
                capacity_loss = torch.nn.ReLU()(kl - kl_cap_current).mean()
                loss += capacity_loss * kl_gamma
        else:
            kl_cont = self.normal1_kl_divergence(mu, log_var)
            kl_disc = self.discrete1_kl_divergence(discs)
            kl = kl_disc + kl_cont
            capacity_loss = 0.0001 * kl.mean()
            loss += capacity_loss
        ...
        rnn_output, _ = self._model['modules']['state_predict_memory'](future_actions[:,:-1], memory_embed.unsqueeze(0))
        ...
        #reconstruction loss
        if RECONSTRUCTION:
            rec_pov = self._model['modules']['generate_pov'](current_embed[:,0])
        ...
        if False:
            ...
            if RECONSTRUCTION:
                plt.imshow(np.concatenate([(states_gen_embed[0,0].permute(1, 2, 0)*0.5+0.5).data.cpu().numpy(),
        ...
        #add diagnostics
        rdict['capacity_loss'] = capacity_loss / np.log(2)
        ...
        if self.auto_capacity:
            rdict['trained'] = str(self.current_capacity / np.log(2)) + " bit"
        else:
            rdict['trained'] = str((self.train_iter * 100.0) / self.capacity[-2]) + " %\t" +\
                str((self.train_iter * (self.capacity[1] / np.log(2))) / self.capacity[-2]) + " bit"
        rdict['actual'] = str(kl.mean().data.cpu().numpy() / np.log(2)) + " bit"
        ...
        if self.auto_capacity:
            if False:
                current_kl = kl.mean().data.cpu().numpy()
                capacity_target = current_kl - self.capacity_delta_target
                self.current_capacity = 0.995 * self.current_capacity + 0.005 * capacity_target
                self.current_capacity = max(self.current_capacity, 0.0)
            else:
                self.current_capacity = 0.995 * self.current_capacity + 0.005 * kl.mean().data.cpu().numpy()
    ...
    def discrete1_kl_divergence(self, log_alpha):
        ...
        kl_loss = kl_values + self.sum_logs
        return kl_loss
    ...
    def select_action(self, state, batch=False):
        ...
        with torch.no_grad():
            ...
            pov_embed_stoch = self._model['modules']['pov_embed'].sample(mu, log_var, discs, training=True)
            #memory_embed
            memory_inp = self.past_embeds[self.past_time].unsqueeze(0)
            memory_embed = self._model['modules']['memory'](memory_inp, pov_embed_stoch)
            self.past_embeds = self.past_embeds.roll(1, dims=0)
            self.past_embeds[0] = pov_embed[0]
            #all embed
            tembed = [pov_embed_stoch, memory_embed]
            for o in self.state_shape:
                if o in self.state_keys and o != 'pov':
