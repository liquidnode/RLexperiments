...
class VisualNet(torch.nn.Module):
    def __init__(self, state_shape, args, cont_dim=None, disc_dims=None, variational=False):
        ...
        encnn_layers = []
        self.embed_dim = sum(disc_dims) + cont_dim
        ...
        print('pov', self.state_shape['pov'])
        ...
        if not NO_LAST_LINEAR:
            ...
            self.embed_dim = in_channels*(end_size**2)
        encnn_layers.append(torch.nn.Sequential(*tencnn_layers))
...
class SimpleBH():
    def __init__(self, state_shape, action_dict, args, time_deltas, only_visnet=False):
        ...
        for i in range(0,len(self.time_deltas)):
            if self.time_deltas[len(self.time_deltas)-i-1] == 0:
                ...
                break
        self.max_predict_range = self.time_deltas[-1]
    ...
    def loadstore(self, filename, load=True):
        if load:
            ...
            state_dict = torch.load(filename + 'simple_BeClone')
            if self.only_visnet:
                state_dict = {n: v for n, v in state_dict.items() if n.startswith('pov_embed.')}
            self.load_state_dict(state_dict)
            if 'Lambda' in self._model:
                self._model['Lambda'] = torch.load(filename + 'simple_BeCloneLam')
        else:
            torch.save(self._model['modules'].state_dict(), filename + 'simple_BeClone')
            torch.save(self.train_iter, filename + 'train_iter')
            if 'Lambda' in self._model:
                torch.save(self._model['Lambda'], filename + 'simple_BeCloneLam')
    ...
    def embed_desc(self):
        desc = {}
        desc['pov_embed'] = {'compression': False, 'shape': [self.continuous_embed_dim], 'dtype': np.float32}
        if len(self.disc_embed_dims) == 0:
            return desc
        else:
            assert False #not implemented
    ...
    def _make_model(self):
        ...
        self.cnn_embed_dim = modules['pov_embed'].embed_dim
        if self.only_visnet:
        ...
        modules['action_predict'] = FullDiscretizerWLast(self.cnn_embed_dim + self.memory_embed + self.state_dim, self.device, self.args, self.action_dict)

        if not NO_VALUE:
            modules['V_value'] = torch.nn.Sequential(torch.nn.Linear(self.cnn_embed_dim + self.memory_embed + self.state_dim, self.args.hidden),
                                                     Lambda(lambda x: swish(x)),
    ...
    def pretrain(self, input, worker_num=None):
        ...
        for k in states:
            if k != 'pov':
                if self.zero_time_point+self.max_predict_range < states[k].shape[1]:
                    next_state[k] = states[k][:,self.zero_time_point+self.max_predict_range]
                else:
                    next_state[k] = states[k][:,self.zero_time_point+1]
        #current_actions = {}
        #for k in actions:
        ...
        last_action = {}
        for a in self.action_dict:
            current_action[a] = actions[a][:,self.zero_time_point]
            last_action['last_'+a] = lastactions['last_'+a][:,self.zero_time_point]
        ...
        bc_loss, _, _, _, _ = self._model['modules']['action_predict'].get_loss_BC(tembed, current_action, last_action, is_weight)
        loss = bc_loss.clone()
        rdict = {}
        ...
        if not NO_VALUE:
            ...
            if is_weight is None:
                V_loss = (current_V - target).pow(2).mean()
            else:
                V_loss = ((current_V - target).pow(2) * is_weight).mean()
            loss += V_loss * 1e-3
            rdict['V_loss'] = V_loss
        else:
        ...
        with torch.no_grad():
            if 'tidxs' in input:
                ...
                rdict['prio_upd'] = [{'error': torch.abs(current_V - target).data.cpu().clone(),
                                      'replay_num': 0,
    ...
    def select_action(self, state, batch=False):
        ...
        with torch.no_grad():
            ...
            r_action = {}
            o_action = {}
            r_action, conaction, entropy = self._model['modules']['action_predict'].sample(tembed, lastaction)
            #check for noop
            is_noop = True
            for a in r_action:
            ...
            for a in o_action:
                if a != 'camera':
                    is_noop = is_noop and (o_action[a] == 0)
                else:
                    is_noop = is_noop and np.all(np.abs(o_action[a]) < 1e-5)
            if is_noop:
                kind = np.random.randint(0, len(o_action))
                if list(o_action.keys())[kind] == 'camera':
                    o_action[list(o_action.keys())[kind]] = np.random.sample((2,)) * 0.1 - 0.05
                else:
                    o_action[list(o_action.keys())[kind]] = np.random.randint(0, self.action_dict[list(o_action.keys())[kind]].n)
            o_action['camera'] = self.map_camera(o_action['camera'])

            #o_action['sneak_sprint'] = 0
...
def main():
    ...
    parser.add_argument("--agent_name", type=str, default='SimpleBH', help="Model name")
    parser.add_argument("--agent_from", type=str, default='complete_behavioral_cloning', help="Model python module")
    parser.add_argument("--needs-next", action='store_true', default=False, help="")
    parser.add_argument("--env", default='MineRLTreechop-v0', type=str, help="Gym environment to use")#MineRLObtainDiamond-v0#MineRLNavigateDense-v0#MineRLTreechop-v0
    ...
    parser.add_argument("--needs_multi_env", default=ALL_TRAIN, action="store_true", help="")
    parser.add_argument("--log_reward", default=ALL_TRAIN, action="store_true", help="")
    ...
    args.needs_embedding = MEMORY_MODULE
    args.needs_last_action = True
    if PRETRAIN:
        args.load = "treechop/model3_"#
        args.save = "treechop/model3_"#
    else:
        args.load = "treechop/model3_"#
        args.save = None#"treechop/model_"#
    ...
    if MEMORY_MODULE:
        ...
        past_times.reverse()
        time_deltas = past_times + [0, time_skip]
        other_past = [-int(delta_a*n**2+n+1) + time_skip for n in range(num_past_times)]
        other_past.reverse()
        time_deltas = other_past + time_deltas
    else:
        num_past_times = 0
        args.num_past_times = 0
        time_deltas = [0, time_skip]
    ...
    if PRETRAIN:
        ...
        replay = VirtualReplayBuffer(args, traj_gen.data_description(), traj_gen.traj_pipe, [t-start for t in time_deltas], 50+end, pov_time_deltas=pov_time_deltas)
