...
def kl_schedule(train_iter, cycle):
    if train_iter < 400 * 10000:
        return ((train_iter + (cycle // 2)) % cycle) / cycle
    else:
        return 1
...
class VirtualPG():
    def __init__(self, state_shape, action_dict, args, time_deltas):
        ...
        self.free_nats = 3.0
        self.cost_of_information_pov = 5.0e-7
        self.cost_of_information_actions = 1.0e-7
    ...
    def loadstore(self, filename, load=True):
        if load:
            ...
            state_dict = torch.load(filename + 'virtualPGmultiaction_var')
            self.load_state_dict(state_dict)
            if 'Lambda' in self._model:
                self._model['Lambda'] = torch.load(filename + 'virtualPGmultiaction_varLam')
        else:
            torch.save(self._model['modules'].state_dict(), filename + 'virtualPGmultiaction_var')
            torch.save(self.train_iter, filename + 'train_iter')
            if 'Lambda' in self._model:
                torch.save(self._model['Lambda'], filename + 'virtualPGmultiaction_varLam')
    ...
    def _make_model(self):
        ...
        #state prediction
        modules['state_predict_hidden'] = torch.nn.Sequential(torch.nn.Linear(self.actions_embed_dim + self.cnn_embed_dim + self.memory_embed + self.state_dim + 1, self.args.hidden),
                                                       Lambda(lambda x: swish(x)),
                                                       torch.nn.Linear(self.args.hidden, self.args.hidden),
                                                       Lambda(lambda x: swish(x)))
        modules['state_predict'] = torch.nn.Linear(self.args.hidden, self.cnn_embed_dim * 2)#mean + variance
    ...
    def pretrain(self, input):
        ...
        pov_embeds_mean, pov_embeds_log_var = self._model['modules']['pov_embed'](states_gen_embed)
        pov_embeds = self._model['modules']['pov_embed'].sample(pov_embeds_mean, pov_embeds_log_var)
        current_pov_embed = pov_embeds[:,0]
        next_pov_embed = pov_embeds[:,1]

        #make memory embeds
        current_memory_embed = self.get_memory_embed(past_embeds, current_pov_embed)
        next_memory_embed = self.get_memory_embed(next_past_embeds, next_pov_embed)

        #get action and aux loss
        loss, current_actions_embed, rdict = self.prediction_loss(current_pov_embed, current_memory_embed, current_state, current_actions, reward[:,self.zero_time_point:])
        

        #pov stuff
        if True:
            #pov embed compression
            pov_kl = self.normal1_kl_divergence(pov_embeds_mean[:,0], pov_embeds_log_var[:,0])
        ...
        #current pov reconstruction
        if False:
            rec_pov = self._model['modules']['generate_pov'](current_pov_embed)
        ...
        state_predict_hidden = self._model['modules']['state_predict_hidden'](torch.cat([current_pov_embed,
                                                                                         current_actions_embed,
                                                                                         current_memory_embed,
                                                                                         cat_current_state,
                                                                                         r_offset], dim=-1))
        pred_inter_pov_embed_mean, pred_inter_pov_embed_log_var = torch.split(self._model['modules']['state_predict'](state_predict_hidden), self.cnn_embed_dim, -1)
        pred_inter_pov_embed = sample_gaussian(pred_inter_pov_embed_mean, pred_inter_pov_embed_log_var)
        if False:
            pov_pred_kl = self.normal2_kl_divergence(pred_inter_pov_embed_mean, pred_inter_pov_embed_log_var, pov_embeds_mean[:,2], pov_embeds_log_var[:,2])#(0.9 * pov_embeds_mean[:,2].detach() + 0.1 * pov_embeds_mean[:,2]), (0.9 * pov_embeds_log_var[:,2].detach() + 0.1 * pov_embeds_log_var[:,2]))
            loss += 1e-2 * pov_pred_kl
            rdict['pred_pov_kl'] = pov_pred_kl / np.log(2)
        
        if False:
            #intermediate pov reconstruction
            inter_rec_pov = self._model['modules']['generate_pov'](pred_inter_pov_embed)
            inter_rec_loss = (self._model['loss'](inter_rec_pov, states_gen_embed[:,2]) * 10.0 + self._model['l1loss'](inter_rec_pov, states_gen_embed[:,2]))
            loss += inter_rec_loss
            rdict['inter_rec_loss'] = inter_rec_loss / np.log(2)
        else:
            #intermediate pov reconstruction
            inter_rec_pov = self._model['modules']['generate_pov'](         pred_inter_pov_embed_mean             )
            inter_rec_loss = (self._model['loss'](inter_rec_pov, states_gen_embed[:,2]) * 10.0 + self._model['l1loss'](inter_rec_pov, states_gen_embed[:,2]))
            loss += inter_rec_loss
            rdict['inter_rec_loss'] = inter_rec_loss / np.log(2)
        ...
        #Q value stuff
        #current Q
        state_predict_hidden_last = self._model['modules']['state_predict_hidden'](torch.cat([current_pov_embed,
                                                                                              current_actions_embed,
                                                                                              current_memory_embed,
                                                                                              cat_current_state,
                                                                                              torch.ones([cat_current_state.shape[0], 1], device=cat_current_state.device, dtype=torch.float32)], dim=-1))
        Q = self._model['modules']['Q_value'](state_predict_hidden_last)

        #next Q
        with torch.no_grad():
            tembed = [next_pov_embed, next_memory_embed]
            for o in self.state_shape:
                if o in self.state_keys and o != 'pov':
                    tembed.append(next_state[o])
            ...
            actions_one_hot = []
            for a in self.action_dict:
                next_action[a] = next_actions[a][:,0]
                if a != 'camera':
            ...
            next_actions_embed_mean, next_actions_embed_log_var = torch.split(self._model['modules']['actions_enc'](tembed_wactions), self.actions_embed_dim, dim=-1)
            next_actions_embed = sample_gaussian(next_actions_embed_mean, next_actions_embed_log_var)
            ...
            state_predict_hidden_last = self._model['modules']['state_predict_hidden'](torch.cat([next_pov_embed.detach(),
                                                                                                  next_actions_embed.detach(),
                                                                                                  next_memory_embed.detach(),
                                                                                                  cat_next_state,
                                                                                                  torch.ones([cat_next_state.shape[0], 1], device=cat_next_state.device, dtype=torch.float32)], dim=-1))
            next_Q = self._model['modules']['Q_value'](state_predict_hidden_last)
            target = (reward[:,self.zero_time_point:(self.zero_time_point+self.max_predict_range)] * self.gammas).sum(1) + (GAMMA ** self.max_predict_range) * next_Q

        Q_loss = (Q - (0.8 * target + 0.2 * Q).detach()).pow(2).mean() * 10.0
        rdict['Q_loss'] = Q_loss
        rdict['Q_mean'] = Q.mean()
        loss += Q_loss
        ...
        if False:
            ...
            aftP = np.concatenate([(states_gen_embed[0,2].permute(1, 2, 0)*0.5+0.5).data.cpu().numpy(),
                                    (inter_rec_pov[0].permute(1, 2, 0)*0.5+0.5).data.cpu().numpy()], axis=1)
    ...
    def prediction_loss(self, current_pov_embed, current_memory_embed, current_state, current_actions, reward):
        ...
        actions_embed = sample_gaussian(actions_embed_mean, actions_embed_log_var)
        ...
        if False:
            ...
            comp_bc_loss, _, _, _, _ = self._model['modules']['actions_dec'].get_loss_BC(actions_embed, current_action)
            loss = comp_bc_loss * 1.2

        #make action embed prediction loss
        ...
        if False:
            pred_actions_kl = self.normal2_kl_divergence(pred_actions_embed_mean, pred_actions_embed_log_var, (0.9 * actions_embed_mean.detach() + 0.1 * actions_embed_mean), (0.9 * actions_embed_log_var.detach() + 0.1 * actions_embed_log_var))
            loss += 1e-2 * pred_actions_kl

        #make direct action prediction loss
        ...
        return loss, actions_embed, {'actions_kl': actions_kl / np.log(2.0),
                                     #'pred_actions_kl': pred_actions_kl / np.log(2.0),
                                     'bc_loss': bc_loss,
